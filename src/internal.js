  ...
  if (!this.options.ajax) {
    ...
    rows.each(
      function() {
        ...
        $.each(
          that.columns, function(i, column) {
...
function renderColumnSelection(actions) {
  if (this.options.columnSelection && this.columns.length > 1) {
    ...
    $.each(
      this.columns, function(i, column) {
        if (column.visibleInSelection) {
          var item = $(
            ...
            .on(
              "click" + namespace, selector, function(e) {
                e.stopPropagation();
...
function renderPaginationItem(list, page, text, markerCss) {
  var that   = this,
      ...
      item   = $(tpl.paginationItem.resolve(values))
        .on(
          "click" + namespace, getCssSelector(css.paginationButton), function(e) {
            e.stopPropagation();
            e.preventDefault();
...
function renderRowCountSelection(actions) {
  ...
  if ($.isArray(rowCountList)) {
    ...
    $.each(
      rowCountList, function(index, value) {
        var item = $(
          ...
          .on(
            "click" + namespace, menuItemSelector, function(e) {
              e.preventDefault();
              ...
              if (newRowCount !== that.rowCount) {
                ...
                $this.parents(menuItemsSelector).children().each(
                  function() {
                    var $item           = $(this),
  ...
  if (rows.length > 0) {
    ...
    $.each(
      rows, function(index, row) {
        ...
        $.each(
          that.columns, function(j, column) {
            if (column.visible) {
...
function registerRowEvents(tbody) {
  ...
  if (this.selection) {
    tbody.off("click" + namespace, selectBoxSelector)
         .on(
           "click" + namespace, selectBoxSelector, function(e) {
             e.stopPropagation();
...
function renderTableHeader() {
  ...
  $.each(
    this.columns, function(index, column) {
      if (column.visible) {
  ...
  if (sorting) {
    ...
    headerRow.off("click" + namespace, sortingSelector)
             .on(
               "click" + namespace, sortingSelector, function(e) {
                 e.preventDefault();
